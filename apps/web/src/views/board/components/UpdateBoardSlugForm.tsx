...
export function UpdateBoardSlugForm({
  ...
  const updateBoardSlug = useMutation({
  ...
  const checkSlugInput = {
    boardSlug: debouncedSlug,
    boardPublicId,
  };
  const checkBoardSlugAvailability = useQuery({
    queryKey: apiKeys.board.checkSlugAvailability(checkSlugInput),
    queryFn: () => api.board.checkSlugAvailability(checkSlugInput),
