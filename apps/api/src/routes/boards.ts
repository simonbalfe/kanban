...
import { convertDueDateFiltersToRanges, generateSlug, generateUID } from "../lib/utils";
import {
  boardTypeSchema,
...
export const boardRoutes = (db: dbClient) =>
  new Elysia({ prefix: "/boards" })
    ...
    .get("/:boardPublicId", async ({ params, query, store, set }) => {
      const userId = store.userId;

      const board = await boardRepo.getBoardIdByPublicId(db, params.boardPublicId);
      if (!board) {
        set.status = 404;
    ...
    .get("/by-slug/:boardSlug", async ({ params, query }) => {
      const dueDateFilters = query.dueDateFilters
        ? convertDueDateFiltersToRanges(query.dueDateFilters)
        : [];
    ...
    .post("/", async ({ body, store, set }) => {
      ...
      if (body.sourceBoardPublicId) {
        const sourceBoardInfo = await boardRepo.getIdByPublicId(db, body.sourceBoardPublicId);
        if (!sourceBoardInfo) {
          set.status = 404;
        ...
        const sourceBoard = await boardRepo.getByPublicId(
          db, body.sourceBoardPublicId, userId,
          { members: [], labels: [], lists: [], dueDate: [], type: sourceBoardInfo.type },
        );
        ...
        if (!isSlugUnique || body.type === "template") slug = `${slug}-${generateUID()}`;
      ...
      if (body.labels.length) {
        const labelInputs = body.labels.map((label: string, index: number) => ({
          publicId: generateUID(),
          name: label,
          boardId: result.id,
          createdBy: userId,
    ...
    .put("/:boardPublicId", async ({ params, body, set }) => {
      const board = await boardRepo.getBoardIdByPublicId(db, params.boardPublicId);
      if (!board) {
        set.status = 404;
        return { error: "Board not found" };
    ...
    .delete("/:boardPublicId", async ({ params, store, set }) => {
      const userId = store.userId;

      const board = await boardRepo.getWithListIdsByPublicId(db, params.boardPublicId);
      if (!board) {
        set.status = 404;
        return { error: "Board not found" };
      }
    ...
    .get("/:boardPublicId/check-slug", async ({ params, query, set }) => {
      const board = await boardRepo.getBoardIdByPublicId(db, params.boardPublicId);
      if (!board) {
        set.status = 404;
        return { error: "Board not found" };
      ...
      const available = await boardRepo.isBoardSlugAvailable(db, query.boardSlug);
